...
public class BoardinsertServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setAttribute("OKMsgKey", msgOK);
		...
		if (parts != null) {
			for (Part p : parts) {
				...
				} else if (p.getContentType().equals("image/jpeg")  || p.getContentType().equals("image/jpg")  ||p.getContentType().equals("image/gif") || p.getContentType().equals("application/x-rar-compressed")) {
		...
		try {
			...
			if(errorMsgMap.isEmpty()) {
				Integer n = bs.insertBoard(bb);
				if(n==1) {
					msgOK.put("BoardInsertOk", "佈告欄新增完成");
					Demoboard = bs.loadbulletinboard_teacher(cb);
					session.setAttribute("Demoboard", Demoboard);
				}else {
					errorMsgMap.put("BoardInsertError", "佈告欄新增失敗");
				}
			}
			if(!msgOK.isEmpty()) {
			...
			e.printStackTrace();
			errorMsgMap.put("errorIDDup", e.getMessage());
